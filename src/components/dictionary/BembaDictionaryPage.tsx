...
function renderExamplesHtml(html: string) {
  return html
    .replace(/<span[^>]*class=bemba_text[^>]*>.*?<\/span>\s*/i, "")
    .replace(
      /class=in_english/g,
      'style="padding:5px 10px;margin:10px auto;word-spacing:1.7px;font-size:17px;text-transform:none;color:rgba(255,255,255,0.9)"'
    )
    .replace(
      /class=in_bemba/g,
      'style="padding:5px 10px;margin:10px auto;word-spacing:1.7px;font-size:17px;text-transform:none;color:rgba(255,255,255,0.9)"'
    )
    .replace(/<span>\s*English&gt;&gt;\s*<\/span>/g, '<span style="padding-right:5px;color:grey;font-size:13px">English&gt;&gt;</span>')
    .replace(/<span>\s*Bemba&gt;&gt;\s*<\/span>/g, '<span style="padding-right:6px;color:grey;font-size:13px">Bemba&gt;&gt;</span>')
    .replace(/<b>/g, '<b style="color:lightblue;font-weight:normal">');
}
...
function getEntrySpeechText(entry: DictEntry) {
  const exampleText = decodeHtmlEntities(entry.html)
    .replace(/<[^>]+>/g, " ")
    .replace(/>>/g, " ")
    .replace(/\s+/g, " ")
    .trim();
  return `${entry.word} : ${entry.bemba}. ${exampleText}`;
}
      ...
      <div style={{ background: "rgba(0,0,0,0.2)", border: "1px solid rgba(255,255,255,0.1)", padding: "16px", borderRadius: "4px" }}>
        ...
        {mode === "solo" && (
          <div className="mb-3 text-sm text-white/80">
            Welcome to the quiz where you will translate English words into Bemba and vice versa. Note the following before starting:
          </div>
        )}
...
export default function DictionaryPage() {
  ...
  // ── On mount
  useEffect(() => {
    fetch(`/api/bemba-dictionary/word-of-the-day`).then(r => r.json()).then(d => setWotd(d));
  ...
  const loadWordListPage = async (page: number, from = sourceLang, to = targetLang) => {
    ...
    const res = await fetch(`/api/bemba-dictionary/words?page=${page}`);
  ...
  const doSearch = useCallback(async (q: string) => {
    ...
    setSearching(true);
    const res = await fetch(`/api/bemba-dictionary/search?q=${encodeURIComponent(q)}`);
    const data = await res.json();
    const results = (data.results ?? []) as DictEntry[];
    setSuggestions(results);
    const normalizedQuery = q.trim().toLowerCase();
    const exactMatch = results.find((entry) => entry.word.toLowerCase() === normalizedQuery || entry.bemba.toLowerCase() === normalizedQuery);
    if (exactMatch) {
      setShowSuggestions(false);
      setSelected(exactMatch);
      setCurrentEntry(exactMatch);
      setEntryHistory((prev) => {
        const next = [...prev, exactMatch];
  ...
  const fetchRandomWord = useCallback(async () => {
    ...
    const res = await fetch(`/api/bemba-dictionary/words?page=${randomPage}`);
    const data = await res.json();
  ...
  const shareDictionary = useCallback(async () => {
    const shareData = {
      title: "Bemba Dictionary",
      text: "Translate between English and Bemba, practice with quizzes, and challenge friends.",
  ...
  const loadAndStartQuiz = async () => {
    setLoadingQuiz(true);
    const res = await fetch(`/api/bemba-dictionary/quiz`);
    const data = await res.json();
  ...
  return (
    <div>
      <div
        className="mb-5 overflow-hidden rounded-[24px] border border-cyan-400/10 bg-slate-950/70 shadow-[0_24px_80px_rgba(0,0,0,0.35)] backdrop-blur"
        style={{ backgroundImage: "radial-gradient(circle at top left, rgba(34,211,238,0.12), transparent 30%), radial-gradient(circle at top right, rgba(59,130,246,0.1), transparent 25%)" }}
      >
        <div className="flex flex-wrap items-center justify-between gap-3 border-b border-white/10 px-4 py-4 sm:px-5">
          <div>
            <h1 className="text-lg font-bold text-white">Bemba dictionary</h1>
          </div>
          <div className="flex flex-wrap items-center gap-1.5 text-xs">
            <button
              type="button"
              onClick={() => switchTab("translate")}
              className={`rounded-full border px-3 py-1.5 transition-all ${tab === "translate" ? "border-cyan-400/40 bg-cyan-400/15 text-cyan-300 shadow-[0_0_0_1px_rgba(34,211,238,0.15)]" : "border-white/10 text-white/65 hover:border-white/20 hover:bg-white/5 hover:text-white"}`}
            >
              Home
            </button>
            <button
              ...
              className={`rounded-full border px-3 py-1.5 transition-all ${tab === "quiz" ? "border-cyan-400/40 bg-cyan-400/15 text-cyan-300 shadow-[0_0_0_1px_rgba(34,211,238,0.15)]" : "border-white/10 text-white/65 hover:border-white/20 hover:bg-white/5 hover:text-white"}`}
            >
              Quiz
            </button>
            <button
              ...
              className={`relative rounded-full border px-3 py-1.5 transition-all ${tab === "challenge" ? "border-cyan-400/40 bg-cyan-400/15 text-cyan-300 shadow-[0_0_0_1px_rgba(34,211,238,0.15)]" : "border-white/10 text-white/65 hover:border-white/20 hover:bg-white/5 hover:text-white"}`}
            >
            ...
            <button
              ...
              className={`relative rounded-full border px-3 py-1.5 transition-all ${tab === "my-challenges" ? "border-cyan-400/40 bg-cyan-400/15 text-cyan-300 shadow-[0_0_0_1px_rgba(34,211,238,0.15)]" : "border-white/10 text-white/65 hover:border-white/20 hover:bg-white/5 hover:text-white"}`}
            >
            ...
            <button
              ...
              className={`rounded-full border px-3 py-1.5 transition-all ${moreMenuOpen ? "border-cyan-400/40 bg-cyan-400/15 text-cyan-300 shadow-[0_0_0_1px_rgba(34,211,238,0.15)]" : "border-white/10 text-white/65 hover:border-white/20 hover:bg-white/5 hover:text-white"}`}
            >
        ...
        <div className="relative p-4 sm:p-5">
          <input
            ...
            placeholder="Search words in bemba or english..."
            className="w-full rounded-full border border-cyan-400/15 bg-black/25 py-3 pl-11 pr-5 text-sm text-cyan-200 outline-none transition-all placeholder:text-white/35 focus:border-cyan-400/35 focus:bg-black/40"
          />
          <i className="fas fa-search pointer-events-none absolute left-8 top-1/2 -translate-y-1/2 text-white/30 text-xs" />
          {searching && <i className="fas fa-spinner fa-spin absolute right-8 top-1/2 -translate-y-1/2 text-cyan-300 text-xs" />}
          {showSuggestions && suggestions.length > 0 && (
            <ul className="absolute left-4 right-4 top-[calc(100%-4px)] z-50 max-h-[320px] overflow-y-auto rounded-[20px] border border-cyan-400/15 bg-slate-950/95 p-2 shadow-[0_24px_60px_rgba(0,0,0,0.45)] backdrop-blur">
              {suggestions.map((s, i) => (
                <li
                  key={i}
                  onClick={() => selectWord(s)}
                  className="cursor-pointer rounded-2xl px-4 py-3 text-sm capitalize text-cyan-100 transition-colors hover:bg-white/5 hover:text-cyan-300"
                >
        ...
        {moreMenuOpen && (
          <div className="grid gap-1 border-t border-white/10 bg-black/15 p-3 text-sm">
            <button type="button" onClick={() => switchTab("challenge")} className="flex items-center gap-3 rounded-2xl px-4 py-3 text-left text-white/70 transition-colors hover:bg-white/5 hover:text-cyan-400">
              <i className="fas fa-users w-4 text-center" />
              Challenge a Friend
            </button>
            <button type="button" onClick={() => switchTab("my-challenges")} className="flex items-center gap-3 rounded-2xl px-4 py-3 text-left text-white/70 transition-colors hover:bg-white/5 hover:text-cyan-400">
              <i className="fas fa-fighter-jet w-4 text-center" />
              Quiz Challenges
            </button>
            <button type="button" onClick={() => switchTab("wotd")} className="flex items-center gap-3 rounded-2xl px-4 py-3 text-left text-white/70 transition-colors hover:bg-white/5 hover:text-cyan-400">
            ...
            <button
              ...
              className="flex items-center gap-3 rounded-2xl px-4 py-3 text-left text-white/70 transition-colors hover:bg-white/5 hover:text-cyan-400"
            >
        ...
        <div>
          ...
          {selected ? (
            <div>
              ...
              <div className="p-4 rounded" style={{ background: "rgba(0,0,0,0.2)", border: "1px solid rgba(255,255,255,0.1)" }}>
                <div className="mb-2 flex items-start justify-between gap-3">
                  <div className="text-[22px] font-bold capitalize text-white">
                    <span>{selected.word}</span>
                    <span className="mx-2 text-white/70">:</span>
                    <span className="text-[20px] font-normal text-white/90">{selected.bemba}</span>
                  </div>
                  <button onClick={() => speak(getEntrySpeechText(selected))} className="text-white/50 hover:text-cyan-400 transition-colors" title="Pronounce">
                ...
                <div
                  ...
                  dangerouslySetInnerHTML={{ __html: renderExamplesHtml(selected.html) }}
                />
              </div>
            </div>
          ) : (
            <>
              ...
              {!search && (
                <>
                  <div className="rounded-[24px] border border-cyan-400/10 p-4 shadow-[0_18px_40px_rgba(0,0,0,0.25)]" style={{ background: "linear-gradient(180deg, rgba(255,255,255,0.05), rgba(0,0,0,0.22))" }}>
                    {currentEntry ? (
                      <div>
                        <div className="rounded-[22px] border border-white/10 px-5 py-5" style={{ background: "rgba(255,255,255,0.04)" }}>
                          <div className="text-[22px] font-bold capitalize text-white">
                            <span>{currentEntry.word}</span>
                            <span className="mx-2 text-white/70">:</span>
                            <span className="text-[20px] font-normal text-white/90">{currentEntry.bemba}</span>
                          </div>
                          <div
                            className="mt-2 text-sm leading-relaxed"
                            style={{ color: "rgba(255,255,255,0.85)", lineHeight: "28px", fontSize: "16px" }}
                            dangerouslySetInnerHTML={{ __html: renderExamplesHtml(currentEntry.html) }}
                          />
                        </div>
                        ...
                        <div className="mt-5 grid grid-cols-[1fr_auto_1fr] items-center gap-3">
                          ...
                          <div className="flex items-center gap-2 rounded-full border border-white/10 bg-black/20 px-3 py-2">
                            <span className="text-xs text-white/40">{speechPending ? "wait.." : speaking ? "playing..." : ""}</span>
                            <button
                              onClick={toggleSpeak}
      ...
      {tab === "wotd" && (
        <div>
          {wotd ? (
            <div className="p-4 rounded" style={{ background: "rgba(0,0,0,0.2)", border: "1px solid rgba(255,255,255,0.1)" }}>
              <p className="text-xs text-white/40 mb-2 italic">Word of the day</p>
              <div className="flex items-center justify-between mb-3">
                <div className="text-[22px] font-bold capitalize text-white">
                  <span>{wotd.word}</span>
                  <span className="mx-2 text-white/70">:</span>
                  <span className="text-[20px] font-normal text-white/90">{wotd.bemba}</span>
                </div>
                <button onClick={() => speak(getEntrySpeechText(wotd))} className="text-white/50 hover:text-cyan-400 transition-colors">
                  <i className="fas fa-volume-up" />
                </button>
              </div>
              <div
                className="text-sm leading-relaxed"
                style={{ color: "rgba(255,255,255,0.85)", lineHeight: "26px", fontSize: "16px" }}
                dangerouslySetInnerHTML={{ __html: renderExamplesHtml(wotd.html) }}
              />
            </div>
          ) : (
          ...
          {randomEntry ? (
            <div className="p-4 rounded" style={{ background: "rgba(0,0,0,0.2)", border: "1px solid rgba(255,255,255,0.1)" }}>
              <div className="flex items-start justify-between mb-2">
                <div className="text-[22px] font-bold capitalize text-white">
                  <span>{randomEntry.word}</span>
                  <span className="mx-2 text-white/70">:</span>
                  <span className="text-[20px] font-normal text-white/90">{randomEntry.bemba}</span>
                </div>
                <button onClick={() => speak(getEntrySpeechText(randomEntry))} className="text-white/50 hover:text-cyan-400 transition-colors" title="Pronounce">
                  <i className="fas fa-volume-up" />
                </button>
              </div>
              <div
                className="text-sm leading-relaxed"
                style={{ color: "rgba(255,255,255,0.85)", lineHeight: "26px", fontSize: "16px" }}
                dangerouslySetInnerHTML={{ __html: renderExamplesHtml(randomEntry.html) }}
              />
            </div>
          ) : (
            <div className="text-center py-8 text-white/40">
